...
apt update
apt install -y build-essential wget dkms autotools-dev apt-utils
...
./mlnxofedinstall --hpc --without-fw-update --force --dkms --add-kernel-support -k 6.12.13-nvidia-gpu-confidential -vvv
